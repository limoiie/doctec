    ...
    try:
        ...
        if file_type == PDF or extension == ".pdf":
            result = [
                filename,
                "PDF",
                int(pdf_detect.detect(filename)[0]),
                pdf_detect.detect(filename)[1],
            ]
        elif file_type == DOCX or extension == ".docx":
            result = [
                filename,
                "DOCX",
                int(docx_detect.detect(filename)),
                docx_detect.describe_chinese(filename),
            ]
        elif file_type == OLE:
            result = [
                filename,
                "OLE",
                int(doc_detect.detect(filename)),
                doc_detect.describe(filename),
            ]
        else:
...
def check_folder_mt(
    folder_path: str, num_threads: int = 4
