...
def PDFiD2String_yl(xmlDoc, nozero, force):
    result = []
    if xmlDoc.documentElement.getAttribute('ErrorOccured') == 'True':
        return result
    if not force and xmlDoc.documentElement.getAttribute('IsPDF') == 'False':
        return result
    result.append(xmlDoc.documentElement.getAttribute('Header'))
    for node in xmlDoc.documentElement.getElementsByTagName('Keywords')[0].childNodes:
        if not nozero or nozero and int(node.getAttribute('Count')) > 0:
            # result += ' %-16s %7d' % (node.getAttribute('Name'), int(node.getAttribute('Count')))
            count = str(node.getAttribute('Count'))
            if int(node.getAttribute('HexcodeCount')) > 0:
                count += '(%d)' % int(node.getAttribute('HexcodeCount'))
            result.append(count)
...
def ProcessFile(filename, options, plugins):
    ...
    if plugins == [] and options.select == '':
        # Print(PDFiD2String_yl(xmlDoc, options.nozero, options.force), options)
        return PDFiD2String_yl(xmlDoc, options.nozero, options.force)
...
def PDFiDMain(filenames, options):
    ...
    for filename in filenames:
        if options.scan:
            ...
            results = ProcessFile(filename, options, plugins)
            return results
...
def run_pdfid(filenames, options):
    ...
    if len(filenames) == 0:
        ...
        try:
            oExpandFilenameArguments = cExpandFilenameArguments(filenames, options.literalfilenames, options.recursedir, False)
            filenames = oExpandFilenameArguments.Filenames()
            ...
            print(e)
            return
    res = PDFiDMain(filenames, options)
    return res
