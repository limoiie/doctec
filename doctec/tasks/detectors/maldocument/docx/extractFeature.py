...
class OfficeFeatureExtractor:
    ...
    def get_all_suspicious_patterns(self):
        ...
        for (
            ...
            olevba.AUTOEXEC_KEYWORDS.items()
        ):  # types:('AutoExec', 'AutoOpen', 'DocumentOpen'),('AutoExit', 'AutoClose', 'Document_Close', 'DocumentBeforeClose')...
            for type in types:
                type = type.lower()
                result["AutoExec"][type] = 0

        for _, types in olevba.SUSPICIOUS_KEYWORDS.items():  # ('Environ',) ,('Open',)
            for type in types:
                type = type.lower()
...
class VbaFeature:
    ...
    def __get_vba_features(self, code):
        ...
        # 长变量名
        vars = re.findall(
            r"(Dim|Public|Private|Static)\s+([_a-zA-Z]?[a-zA-Z]+[0-9]*[a-zA-Z]*)\(?\)?\s+As\s+.*?",
            code,
        ...
        # 长函数名
        funcs = re.findall(
            r"\s*Sub\s+([_a-zA-Z]?[a-zA-Z]+[0-9]*[a-zA-Z]*)\s*\(.*?\)",
            code,
            re.MULTILINE | re.DOTALL,
        )
        funcs2 = re.findall(
            r"\w\s+Function\s+([_a-zA-Z]?[a-zA-Z]+[0-9]*[a-zA-Z]*)\s*\(.*?\)",
            code,
            re.MULTILINE | re.DOTALL,
        )
        funcs.extend(funcs2)
        ...
        for str in long_strs:
            ...
            # 函数名在长字符串中出现
            for func in funcs:
                if not func in self.funcs_in_str:
        ...
        cir = re.findall(
            r"(For Each|Exit For|do while)", code, re.MULTILINE | re.DOTALL
        )
